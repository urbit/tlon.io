...
This is Tlon's guide for new developers that want to build applications on Urbit. The goal of this guide is to take you from a fresh machine to a working development environment. We assume basic knowledge of the command line and somewhat prioritize macOS, but Linux is fully supported (see the Urbit foundation docs for more details). It's important that this guide remains clear and correct, if you find something broken please let us know at support@tlon.io

>  The following assumes you have an urbit binary installed and the git repo cloned. Follow the steps in External Pre-Requisites before continuing if you do not.
...
#### What is a "fake" ship?
  ...
  For local development we'll create a 'fake' ship - a local ship that is not connected to the Urbit network. This means we don't need a real ID (eg comet, planet) and can delete and recreate the ship whenever we want to reset things.
...
>  To exit the dojo type `|exit` or `ctrl-D` (on a blank line).
...
###### With Citadel
The Citadel app allows you to create and manipulate your desks purely from the dojo. It provides a repl and a split screen front end that works with the docs app.
 >  (You might be reading this on urbit already - skip the following if so)
- Install on the livenet from `~mipber/citadel`, either via the UI or by typing `|install ~mipber %citadel` into the dojo. Once it finishes, type `|link %citadel-cli` to switch to a repl with custom commands reached through the `;` prefix. These allow you to create desks from examples or simply use another desk as a starting point.
- Install `~pocwet/docs` to read this guide natively in urbit.
- In the `citadel` repl:
      ```
         ;desk hello ~ hel
      ```

   This will create a desk `%hello` from the `hel` template, which contains all the dependencies we need to create a simple 'hello world' app (along with a sample gall agent).
- In the `dojo`:
    ```
    |mount %hello
    ```

This will mount the files to a directory on the host filesystem so they can be edited using your preferred editor.
...
### Making an App
 ...
 - Now we should see `hello` inside the `zod` directory, populated with files.
   ...
   - The optional `desk.ship` file specifies the original publisher of this desk. Make sure you set this to our fake ship `~zod`. In practice you'll set this to the ID you want to be seen as the publisher (likely your own Urbit ID). Currently there is no verification that makes sure publishers are honest about this, but eventually there will be.
     ```bash
